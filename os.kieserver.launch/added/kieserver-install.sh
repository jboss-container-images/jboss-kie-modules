...
if [ -d ${DEPLOY_DIR} ]; then
    ...
    for JAR in $(find ${DEPLOY_DIR}/ -maxdepth 1 -name *.jar | sed 's|.*/||'); do
        ...
        if [ -e "${POM}" ]; then
            ...
            # Discover KIE_CONTAINER_DEPLOYMENT for when env var not specified, only kjar (has kmodule.xml) should be configured.
            if [ "${KJAR}" != "" ]; then
                pushd $(dirname ${POM}) &> /dev/null
                    ...
                    log_info "Adding ${kieContainerDeployment} to ${kieContainerDeploymentsFile}"
