...
Feature: IBM BAMOE Business Central configuration tests
  ...
  Scenario: Check if image version and release is printed on boot
    ...
    Then container log should contain ibm-bamoe/bamoe-businesscentral-rhel8 image, version
  ...
  Scenario: Check for product and version environment variables
    ...
    Then run sh -c 'echo $JBOSS_PRODUCT' in container and check its output for ibm-bamoe-businesscentral
     And run sh -c 'echo $IBM_BAMOE_BUSINESS_CENTRAL_VERSION' in container and check its output for 8.0
