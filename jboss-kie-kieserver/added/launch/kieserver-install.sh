...
install_jar_pom() {
    ...
    pom_artifact=$(echo "${pom}" | sed 's|/pom\.xml$||' | sed 's@.*/@@')
    ...
    if [ -e "${temp_jars_dir}/${jar}/META-INF/kmodule.xml" ]; then
        ...
        pushd "$(dirname "${pom}")" || exit &> /dev/null
            # Add JVM default options
            export MAVEN_OPTS="${MAVEN_OPTS:-$(/opt/run-java/java-default-options)}"
            # Use maven batch mode (CLOUD-579)
            ...
            kjarGroupId="$(mvn help:evaluate -Dexpression=project.artifact.groupId "${maven_args_evaluate}" | grep -E -v '(^\[.*\])|(Download.*: )')"
            kjarArtifactId="$(mvn help:evaluate -Dexpression=project.artifact.artifactId "${maven_args_evaluate}" | grep -E -v '(^\[.*\])|(Download.*: )')"
            kjarVersion="$(mvn help:evaluate -Dexpression=project.artifact.version "${maven_args_evaluate}" | grep -E -v '(^\[.*\])|(Download.*: )')"
...
if [ -d "${DEPLOY_DIR}" ]; then
    ...
    if [ -f "${LOCAL_SOURCE_DIR}/pom.xml" ]; then
        ...
        if grep -qE '<module>.*</module>' "${LOCAL_SOURCE_DIR}/pom.xml" ; then
